...
# Rscript run_bayesspace.R  ../dataset/breast_invasive_carcinoma 8 ./HBC_BayesSpace Visium_FFPE_Human_Breast_Cancer_filtered_feature_bc_matrix.h5
...
show_BayesSpace <- function(input_root, n_cluster, savepath, h5_name){
    ...
    ab <- colSums(dlpfc@assays$Spatial@counts)
    library(rhdf5)
    
    foo1 <- dlpfc@meta.data
    dlpfc@meta.data = foo1[,c('sx','sy','px','py')]
    X <- GetAssayData(dlpfc, slot = "counts", assay = "Spatial")

    dlpfc <- SingleCellExperiment(assays = list(counts = X), colData = dlpfc@meta.data)
    ...
    write.table(reducedDim(dlpfc, "PCA"), file=file.path(output, 'bayesSpace_PCA_embeddings.csv'), sep='\t', quote=FALSE)
