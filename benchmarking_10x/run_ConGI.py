...
parser.add_argument('--path', type=str, default="../dataset/10x/")
...
parser.add_argument('--name', type=str, default="151676")
...
# dataset
trainset = Dataset(dataset, path, "id","name_id","num_fov",name, gene_preprocess=gene_preprocess, n_genes=n_gene,
...
testset = Dataset(dataset, path, "id","name_id","num_fov",name, gene_preprocess=gene_preprocess, n_genes=n_gene,
                   prob_mask=prob_mask, pct_mask=pct_mask, prob_noise=prob_noise, pct_noise=pct_noise, sigma_noise=sigma_noise,
...
# train
trainer = TrainerSpaCLR(args, "num_fov","path","id",trainset.n_clusters, network, optimizer, log_dir, device=device)
