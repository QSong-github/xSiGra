...
def test_explanations(
    ...
    loader = DataLoader(datas, batch_size=1, num_workers=1, shuffle=False)
    ...
    for i, batch in enumerate(loader):
        ...
        y_prob = torch.sigmoid(out.cpu().detach()).tolist()
        probs.extend(y_prob)

        pred = np.round(np.asarray(y_prob))
        outlabel.extend(pred)
    ...
    score = f1_score(gtlabel, outlabel, average="weighted")
    print("F1 score train:", score)

    auc = roc_auc_score(gtlabel, probs)
    print("AUC:", auc)
...
obs_df = adata_pred.obs.dropna()
device = torch.device(opt.device if torch.cuda.is_available() else "cpu")
linearmodel = ClusteringLayer(1).to(device)
...
cross_el = torch.nn.BCEWithLogitsLoss()
...
for epoch in range(epochs):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        loss = cross_el(out.squeeze(), labels.float().to(device))
    ...
    print("AUC:", roc_auc_score(gtlabel, probs))

if not os.path.exists("./saved_model1/" + opt.dataset + "_" + opt.benchmark):
    os.makedirs("./saved_model1/" + opt.dataset + "_" + opt.benchmark)
torch.save(
    linearmodel.state_dict(),
    "./saved_model1/" + opt.dataset + "_" + opt.benchmark + "/linearmodel_gt_mask.pth",
)
...
torch.save(
    linearmodel.state_dict(),
    "./saved_model1/" + opt.dataset + "_" + opt.benchmark + "/linearmodel_gt_all.pth",
)
...
for idx, batch in enumerate(test_loader):
    ...
    correct += torch.sum(torch.FloatTensor(pred) == labels).item()
    total += labels.size(0)
...
if not os.path.exists("./fidelity_results1/"):
    os.makedirs("./fidelity_results1/")
results_df = pd.DataFrame.from_dict({"celltype": celltypes, "accuracy": accuracies, "F1 score": scores, "auc": aucs})
results_df.to_csv("./fidelity_results1/fidelity_mask_" + opt.benchmark + "_" + opt.dataset)
