...
ids = ["fov6", "fov7", "fov8", "fov9", "fov10"]
img_names = ["F006", "F007", "F008", "F009", "F010"]
adatas = list()
exp = pd.DataFrame()
...
def check(start, end, scores, pairs1, id, lr_df1, lr_df2, df_test1):
    # Read known LR pairs
    lr = pd.read_csv("LRdb_saved_files.csv")
    ...
    # Select genes that are present in input gene expression
    ...
    for i in comb:
        ...
        if len(final_df1) != 0:
            ...
            cci_results = pd.concat([df_test1, final_df1])
            ...
            lr_df1 = pd.concat([lr_df1, score1], axis=0)
            lr_df2 = pd.concat([lr_df2, score2], axis=0)
        ...
        del celltypeimp1
        del celltypeimp2
    return lr_df1, lr_df2, df_test1


# Read known ligand receptor pairs
lr = pd.read_csv("LRdb_saved_files.csv")
scores = []
pairs = []
...
lr_score1, lr_score2, cci_results = check(0, 500, scores, pairs, 1, lr_score1, lr_score2, cci_results)
lr_score1, lr_score2, cci_results = check(500, 1000, scores, pairs, 2, lr_score1, lr_score2, cci_results)
lr_score1, lr_score2, cci_results = check(1000, 1500, scores, pairs, 3, lr_score1, lr_score2, cci_results)
lr_score1, lr_score2, cci_results = check(1500, 2000, scores, pairs, 4, lr_score1, lr_score2, cci_results)
lr_score1, lr_score2, cci_results = check(2000, 2500, scores, pairs, 5, lr_score1, lr_score2, cci_results)
lr_score1, lr_score2, cci_results = check(2500, 3500, scores, pairs, 6, lr_score1, lr_score2, cci_results)


if not os.path.exists("./enrichment_results/"):
    os.makedirs("./enrichment_results/")
# Save the data to file for debugging
cci_results.to_csv("./enrichment_results/fov678910.csv")
