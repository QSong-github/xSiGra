...
ids = ["fov1", "fov2", "fov3", "fov4", "fov5"]
img_names = ["F001", "F002", "F003", "F004", "F005"]
adatas = list()
exp = pd.DataFrame()
...
def check(start, end, scores, pairs1, lr_score1, lr_score2, cci_results):
    ...
    # Multiply by number of cells to create proper dataframe for further processing
    ...
    # For each combination compute raw expression-based and importance score-beased cell cell interaction scores
    for i in comb:
        print(i)
...
# Computing seprately for small subset of lr pairs to avoid out of memory error
...
cci_results.to_csv("./enrichment_results/fov12345.csv")
